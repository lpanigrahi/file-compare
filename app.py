...
def dataset_display(file1, file2):
    ...
    if (split_tup_1[1] and split_tup_2[1]) == '.xlsx':
        try:
            ...
            for i in range(len(df2.index)):
                dict2[list(df2[i:i+1].ACCOUNT_NAME)[0]
                      ].append((list(df2[i:i+1]['ENTITLEMENT VALUE'])[0]))

            df1 = pd.read_excel(file1.name, dtype=str)
            for i in range(len(df1.index)):
                dict1[list(df1[i:i+1].UNAME)[0]
                      ].append((list(df1[i:i+1].AGR_NAME)[0]))
    ...
    out2 = pd.DataFrame(
        rows, columns=['UNAME', 'ACCOUNT_NAME', 'AGRNAME', 'MISSING ENTITLEMENT VALUE'])
...
with gr.Blocks() as demo:
    ...
    with gr.Tab("Upload the files"):
        gr.Markdown("##")
        with gr.Row():
            inp1 = gr.File(label="Input File-1 / SAP_SP1_Data",
                           file_types=['.xlsx', '.csv', '.txt'], interactive=True)
            inp2 = gr.File(label="Input File-2 / AccessNow_Data",
                           file_types=['.xlsx', '.csv', '.txt'], interactive=True)
        ...
        with gr.Row():
            gen_btn.click(
                ...
                outputs=[gr.Dataframe(
                    ...
                    headers=['UNAME', 'ACCOUNT_NAME',
                             'AGRNAME', 'MISSING ENTITLEMENT VALUE'],
